...
const IssueCard = ({ issue, onResolve, isResolved, onClose, onUpdateIssue }) => {
  ...
  const [actionAdded, setActionAdded] = useState(actions.length > 0); // Check if action already exists
  ...
  const handleAddAction = (actionData) => {
    ...
    setActionAdded(true); // Disable further action addition
        ...
        <div className="actions-list">
          ...
          {actions.map((action, index) => (
            <div key={index} className="action-item">
              ...
              <p><strong>SysAdmin:</strong> {action.person}</p>
